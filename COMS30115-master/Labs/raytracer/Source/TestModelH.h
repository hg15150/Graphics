...
Material Gloss = { .type = GlossType, .amb = 0.3, .diff = 0, .spec = 1 , .shi = 10 };
Material Fancy = { .type = FancyType, .amb = 0.3, .diff = 0, .spec = 1 , .shi = 15 };
Material Glass = { .type = GlassType, .amb = 0.3, .diff = 0.5, .spec = 0.2 , .shi = 15 };
Material Rough = {  .type = RoughType, .amb = 0.2, .diff = 0.8, .spec = 0.0, .shi = 2};
...
void LoadTestModel( vector<Item*>& triangles )
{
   ...
   //Sphere
   // triangles.push_back( new Sphere( white, Mirror, vec4( 0.5, 0.5, 0, 1 ), 0.35, 0 ) );      //Bottom right
   triangles.push_back( new Sphere( blue, Gloss, vec4( -0.5, 0.3, -0.6, 1 ), 0.35, 1 ) );     //Bottom left
   // triangles.push_back( new Sphere( white, Mirror, vec4( 0.5, -0.5, 0, 1 ), 0.35, 2 ) );     //Top right
   triangles.push_back( new Sphere( blue, Mirror, vec4( 0.5, 0, -0.2, 1 ), 0.35, 3 ) );    //Top left
   ...
   // Front
	// triangles.push_back( new Triangle(E,B,A,red, Mirror) );
	// triangles.push_back( new Triangle(E,F,B,red, Mirror) );
  //
	// // Right
	// triangles.push_back( new Triangle(F,D,B,red, Mirror) );
	// triangles.push_back( new Triangle(F,H,D,red, Mirror) );
  //
	// // BACK
	// triangles.push_back( new Triangle(H,C,D,red, Mirror) );
	// triangles.push_back( new Triangle(H,G,C,red, Mirror) );
  //
	// // LEFT
	// triangles.push_back( new Triangle(G,E,C,red, Mirror) );
	// triangles.push_back( new Triangle(E,A,C,red, Mirror) );
  //
	// // TOP
	// triangles.push_back( new Triangle(G,F,E,red, Mirror) );
	// triangles.push_back( new Triangle(G,H,F,red, Mirror) );
